...
module.exports = (app, Url) => {
  ...
  app.post("/shorten", async (req, res) => {
    ...
    if (id.length < 3) {
      res.json({
        error:
          "Invalid ID. Id's must be above 3 characters long and contain only alphanumeric characters.",
      })
      return
    }
    if (!req.body.url) {
    ...
    urlwohttp =
      split.length > 2
        ? `${split[split.length - 1]}.${split[split.length]}`
        : urlwohttp
  ...
  app.get("/:id", async (req, res) => {
    updateUrls()
    var to = urls.find(
      (i) => (i.id ? i.id.toLowerCase() : "") == req.params.id.toLowerCase()
    )
    if (to) {
      res.redirect(to.url)
      ...
      let to = urls
        .map((i) => i.id)
        .filter((i) => _$.jaroDistance(i, req.params.id) > 0.9)
        .reduce(
          (i, v) =>
            _$.jaroDistance(i, req.params.id) >
            _$.jaroDistance(v, req.params.id)
              ? i
              : v,
          ""
        )
      if (to) {
        res.redirect(urls.find((i) => i.id == to).url)
        Url.findOneAndUpdate(
          {
            id: {
    ...
    if (!url) {
      let out = { error: "No URL with that ID." }
      let closest = (await Url.find({}).lean())
        .map((i) => i.id)
        .filter((i) => _$.jaroDistance(i, req.params.id) > 0.9)
        .reduce(
          (i, v) =>
  ...
  app.put("/update", async (req, res) => {
    ...
    try {
      const id = req.body.url_id
        ? req.body.url_id.replace(/[^a-z0-9]/gi, "")
        : ""
      var url =
        urls.find((i) => i.id == id) || (await Url.findOne({ id: id }).lean())
      if (!url) {
  ...
  function u(req) {
    ...
    return (
      hash(half(ua)[0]).toString(36) +
      hash(half(ua)[1]) +
      hash(req.params.id).toString(36)
    )
  }
